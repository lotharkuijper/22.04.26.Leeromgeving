// Pure helpers voor PDF-pagina-toewijzing aan chunks (server-kant). Bewust géén
// pdfjs-import hier zodat de functies los testbaar blijven in een Node-omgeving.
// Dit is de server-spiegel van src/services/pdf-pages.ts: houd beide in sync.
 ...
 * pageStart/pageEnd (1-based) in chunk.metadata. `chunks` is een array van
 * objecten met een `text`-veld en een `metadata`-object dat ter plekke wordt
 * aangevuld. Bij twijfel laten we de paginavelden bewust weg (geen fout nummer).
 */
export function assignPdfPages(pageTexts, chunks) {
  const pageBounds = [];
  let normFull = '';
