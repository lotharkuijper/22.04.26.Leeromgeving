...
- **The CLIENT-side reprocess swap is NOT atomic — only the 429 window is closed, not the whole swap.** The client PDF reprocess (`retryFailedDocument`) now embeds ALL chunks first and only then does delete-old → batched-insert-new, so an Azure 429 mid-embed can no longer empty a doc. But that delete→insert window itself is plain PostgREST, so a browser close / network drop / RLS failure between the delete and the inserts can still leave 0 or partial chunks. A `destructiveStarted` flag distinguishes pre-swap failures (restore original status, chunks intact) from post-swap failures (mark `failed`).
  - **Why:** there is no client-side DB transaction; only the server's `persistChunksAtomic` (pgPool) is truly atomic. The crash-safety work scoped to the rate-limit case deliberately did not move client persistence server-side.
  - **How to apply:** the real fix for "never empty on any failure" is a server endpoint/RPC that reuses `persistChunksAtomic` for the client reprocess path too; until then the swap window remains best-effort.
