...
Chat/completions: Azure OpenAI (resource `leap-openai-vu`, deployment `gpt-5.5`) — alle chat-calls in `server/index.js` lopen via `OPENAI_CHAT_URL` (Azure-URL) + `chatAuthHeaders()` (`api-key`-header). Embeddings: publieke OpenAI `text-embedding-3-small` (`OPENAI_API_KEY`, Bearer) — nog niet op Azure (apart traject).
