...
export function ResetPasswordPage() {
  ...
  // We wachten tot Supabase de recovery-sessie uit de URL heeft verwerkt.
  ...
  useEffect(() => {
    let mounted = true;
    // Supabase verwerkt de recovery-token in de URL-hash automatisch
    // (detectSessionInUrl) en zendt een PASSWORD_RECOVERY-event uit. We
    // controleren ook direct op een bestaande sessie voor het geval het
    // event al gepasseerd is voordat deze listener er was.
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!mounted) return;
      setHasRecoverySession((prev) => prev ?? !!session);
    });
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-gray-600 text-center">{t('reset.subtitle')}</p>
            <button
              data-testid="button-lang-toggle-reset"
          ...
          {success ? (
            <div className="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-lg text-sm" data-testid="text-reset-success">
              {t('reset.success')}
            </div>
          ) : hasRecoverySession === false ? (
            <div className="space-y-4">
              <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-lg text-sm" data-testid="text-reset-no-session">
                {t('reset.noSession')}
              </div>
              <button
