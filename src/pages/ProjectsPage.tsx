...
export function ProjectsPage() {
  ...
  const archiveProjectRequest = async (
    sessionId: string,
    title: string,
    force: boolean,
    ...
    if (res.status === 409) {
      ...
      const existingDate = body?.existingEntry?.created_at;
      if (!force && existingDate) {
        setArchiveDialog(null);
        setDuplicateConfirm({ sessionId, title, existingDate });
        return 'duplicate';
      }
      throw new Error(body.message || body.error || 'Er bestaat al een projectreflectie.');
    }
    if (!res.ok) {
    ...
    setArchiveNotice({
      kind: 'success',
      message: `Een samenvatting van "${title}" is in je leerdagboek gezet. Je vindt hem terug onder het vak "Projecten".`,
    });
    return 'ok';
  };
  ...
  const handleArchiveProject = async () => {
    ...
    const { sessionId, title } = archiveDialog;
    setArchiving(true);
    try {
      const result = await archiveProjectRequest(sessionId, title, false);
      if (result === 'ok') setArchiveDialog(null);
    } catch (err) {
  ...
  const handleConfirmDuplicateArchive = async () => {
    if (!duplicateConfirm || archiving) return;
    const { sessionId, title } = duplicateConfirm;
    setArchiving(true);
    try {
      await archiveProjectRequest(sessionId, title, true);
  ...
  const formatDutchDateTime = (iso: string): string => {
    try {
    ...
    <div className="space-y-6">
      ...
      {duplicateConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6" data-testid="dialog-duplicate-reflection">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2 bg-amber-100 rounded-xl">
                <BookText className="w-5 h-5 text-amber-700" />
              </div>
              <h2 className="text-lg font-semibold text-gray-900">Er staat al een reflectie</h2>
              <button
                onClick={() => !archiving && setDuplicateConfirm(null)}
                className="ml-auto p-1 rounded hover:bg-gray-100 text-gray-500"
                data-testid="btn-duplicate-reflection-cancel"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <p className="text-sm text-gray-600 mb-2">
              Voor project <strong>"{duplicateConfirm.title}"</strong> staat al een projectreflectie in je leerdagboek (van{' '}
              <strong data-testid="text-existing-reflection-date">{formatDutchDateTime(duplicateConfirm.existingDate)}</strong>).
            </p>
            <p className="text-sm text-gray-600 mb-6">
              Wil je toch een nieuwe samenvatting laten maken? De oude blijft staan; er komt een tweede notitie naast.
            </p>

            <div className="flex flex-col gap-3">
              <button
                onClick={handleConfirmDuplicateArchive}
                disabled={archiving}
                data-testid="btn-duplicate-reflection-confirm"
                className="px-4 py-3 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-semibold rounded-xl hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"
              >
                <BookText className="w-4 h-4" />
                {archiving ? 'Bezig met opslaan...' : 'Ja, sla nogmaals op'}
              </button>
              <button
                onClick={() => setDuplicateConfirm(null)}
                disabled={archiving}
                data-testid="btn-duplicate-reflection-dismiss"
